...
const std::string GetFoodStr(const Food &food)
{
    return std::visit(overloaded{
        ...
        [](unsigned int arg) {
            return (arg >= food::KnownFood.size())
                   ? "unknown something"
...
{
public:
    IAnimal(const char *label, const char *name)
        : m_label{label}
        , m_name{name}
    {}
    ...
    void Eat(const Food &food) override
    {
        ...
        if (it == std::end(eatable))
        {
            std::cerr
                << Tag << ": "
                << m_label << " " << m_name
                << " don't want to eat " << foodName << ": "
                << err::exception::FoodNotSuitable().what()
                << std::endl;
...
{
public:
    Cat(const char *label, const char *name)
        : IAnimal(label, name)
    {}
...
const std::vector<std::string_view> Cat::Eatable = {
    std::string_view{food::Fish},
    std::string_view{food::Milk}
};
...
{
public:
    Dog(const char *label, const char *name)
        : IAnimal(label, name)
    {}
...
/***************************************************************
 *                  CatDog class implementation                *
 ***************************************************************/
class CatDog : public IAnimal<CatDog>
{
public:
    CatDog(const char *label, const char *name, bool isDog)
        : IAnimal(label, name),
          m_isDog{isDog}
    {}
...
/***************************************************************
 *                Something class implementation               *
 ***************************************************************/
class Something : public Animal
{
public:
    Something(const char *label, const char *name)
        : m_label{label}
        , m_name{name}
    {}
    ...
    void Say() override
    {
        std::cerr
            << Tag << ": "
            << m_label << " " << m_name << ": "
            << err::exception::AnimalNotSupported().what()
            << std::endl;
