...
int main([[maybe_unused]] int argc, [[maybe_unused]] const char *argv[])
{
    ...
    {
        std::cerr << Tag << ": " << "Cannot create an execution manager\n";
        return EXIT_FAILURE;
    }
    ...
    {
        std::cerr << Tag << ": " << "Cannot get an application controller\n";
        return EXIT_FAILURE;
    }
    ...
    {
        std::cerr << Tag << ": " << "Cannot start the application from " << appPath << std::endl;
        return EXIT_FAILURE;
    }

    std::cout << Tag << ": " << "Application started with sid " << result.sid << std::endl;
    ...
    {
        std::cerr << Tag << ": " << "Cannot get a notification from the application1\n";
        return EXIT_FAILURE;
    }
    ...
    info2.envs       = std::vector<std::string>{"VFS_FILESYSTEM_BACKEND=client:kl.VfsRamFs"};

    if (ac->StartEntity(app2Path, info2, result) != result_code::Ok)
    {
        std::cerr << Tag << ": " << "Cannot start the application from " << app2Path << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (WaitForNotification() != result_code::Ok)
    {
        std::cerr << Tag << ": " << "Cannot get a notification from the application2\n";
        return EXIT_FAILURE;
    }
    ...
    {
        std::cerr << Tag << ": " << "Cannot stop the application from " << appPath << std::endl;
        return EXIT_FAILURE;
    }

    if (ac->StopEntity(secondAppId) != result_code::Ok)
    {
        std::cerr << Tag << ": " << "Cannot stop the application from " << app2Path << std::endl;
        return EXIT_FAILURE;
    }
