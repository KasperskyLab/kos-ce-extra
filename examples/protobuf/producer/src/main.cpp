...
#include <protobuf_example/Producer.edl.cpp.h>
...
{
    // Verify that the version of the library that we linked with is
    ...
    if (argc == 1)
    {
        LOG(INFO, "No address book file passed - nothing to do.");
        return EXIT_SUCCESS;
    }
    ...
    try
    {
        ...
        auto consumer{app.MakeProxy<ipc::IConsumer>(
            ConnectStaticChannel(consumer::ServiceId, consumer::Endpoint))};
        for (auto file = argv + 1; *file != nullptr; ++file)
        {
            std::ifstream ifs{*file};
            if (!ifs)
            {
                LOG(WARNING, "Address book file {} not found.", *file);
                continue;
            }
            ...
            if (addressBook.SerializeToOstream(&os))
            {
                ...
                if (message.empty())
                {
                    LOG(WARNING, "Address book from file {} is empty.", *file);
                }
                else if (message.size() > ipc::MaxMessageSize)
                {
                    LOG(WARNING, "Address book from file {} is too big.", *file);
                }
                else
                {
            ...
            {
                LOG(ERROR, "Failed to serialize address book from file: {}", *file);
            }
        }
    }
