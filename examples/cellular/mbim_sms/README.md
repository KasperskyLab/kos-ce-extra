# SMS Messaging via MBIM
This example demonstrates how to send SMS messages using the MBIM (Mobile Broadband Interface Model)
network driver in KasperskyOS. The client application encodes a predefined text message into a PDU
(Protocol Data Unit) format and transmits it to a compatible LTE modem via the KasperskyOS MBIM API.
...
# Table of contents
- [SMS Messaging via MBIM](#sms-messaging-via-mbim)
...
* `Client`—Program that sends SMS messages through a modem supporting the MBIM protocol. It encodes
  a text message into PDU format and sends it through the modem.
...
> [!IMPORTANT]
> The example does not define a destination phone number. To send an SMS, you must define the
> `DEST_NUMBER` variable in the [`./client/src/client.c`](client/src/client.c) source file.
