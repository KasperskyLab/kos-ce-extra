            ...
            switch (x.reply.id)
            {
                case AMQP_CONNECTION_CLOSE_METHOD:
                {
                    ...
                    throw std::runtime_error(
                        context + ": server connection error " + std::to_string(m->reply_code)
                        + "h, message: "
                ...
                case AMQP_CHANNEL_CLOSE_METHOD:
                {
                    ...
                    throw std::runtime_error(
                        context + ": server channel error " + std::to_string(m->reply_code)
                        + "h, message: "
                        + std::string{static_cast<char *>(m->reply_text.bytes), m->reply_text.len});
                }
                default:
                    std::stringstream msg;
                    msg << context << ": unknown server error, method id 0x" << std::hex
                        << std::setw(8) << std::setfill('0') << x.reply.id;
