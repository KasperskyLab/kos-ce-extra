...
Consumer::Consumer(const char* hostname, int port): m_conn{amqp_new_connection()}
{
    if (!m_conn)
    {
...
void Consumer::DeclareQueue()
{
    amqp_queue_declare_ok_t *r =
        amqp_queue_declare(m_conn, m_chanel, amqp_empty_bytes, 0, 0, 0, 1, amqp_empty_table);
    ...
    amqp_basic_consume(m_conn, m_chanel, m_queueName, amqp_empty_bytes, 0, 1, 0, amqp_empty_table);
...
void Consumer::RecvAndPrintData()
{
    while (true)
    {
        ...
        std::string exchBytes{static_cast<char *>(envelope.exchange.bytes), envelope.exchange.len};

        std::string rKeyBytes{
            static_cast<char *>(envelope.routing_key.bytes), envelope.routing_key.len};

        std::cout << "Delivery " << envelope.delivery_tag << ", exchange: " << exchBytes
                  << ", routingkey: " << rKeyBytes << std::endl;
